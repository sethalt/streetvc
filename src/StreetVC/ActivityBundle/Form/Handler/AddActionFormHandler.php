<?php
namespace StreetVC\ActivityBundle\Form\Handler;
...
class AddActionFormHandler
{
    ...
    public function handle(Form $form, Request $request)
    {
        ...
        $subject = $this->actionManager->findOrCreateComponent($this->extractComponent($data->subject));
    ...
    protected function extractComponent($component)
    {
        ...
        switch ($model) {
            case 'User':
                $entity = $this->objectManager->getRepository('StreetVCUserBundle:User')->find($identifier);
                break;
            case 'Business':
                $entity = $this->objectManager->getRepository('StreetVCBorrowerBundle:Business')->find($identifier);
                break;
                /*
                list ($brand, $model) = $identifier = explode('-', $identifier);

                $entity = $this->objectManager->getRepository('StreetVCBorrower:Business')->find(array(
                    'brand' => $brand,
                    'model' => $model
                ));
                break;
